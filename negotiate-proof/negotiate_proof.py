...
async def proof_negotiation(params):
    ...
    try:
        ...
        # 1.
        print_log('\n opening a new local pool ledger configuration that will be used '
        ...
        # 2.
        print_log('\n Open pool ledger and get the handle from libindy\n')
        pool_handle = await pool.open_pool_ledger(config_name=pool_name, config=None)

        # 3.
        print_log('\n Creating Issuer wallet and opening it to get the handle.\n')
        ...
        schema = {
            'name': 'gvt',
            'version': '1.0',
            'attributes': '["email", "name", "phone", "genda"]'
        }
        ...
        cred_values_json = json.dumps({
            "email": {"raw": email_new, "encoded": "5944657099558967239210949258394887428692050081607692519917050011144233"},
            "name": {"raw": name_new, "encoded": "1139481716457488690172217916278103335"},
            "phone": {"raw": phone_new, "encoded": "175"},
            "genda": {"raw": genda_new, "encoded": genda_new}
        })
        (cred_json, _, _) = \
        ...
        assert name_new == proof['requested_proof']['revealed_attrs']['attr1_referent']["raw"]
        # return("VP CODE:\n" + proof)
        # # 21.
        ...
        # return("VP CODE:" ,trust_anchor_did)
